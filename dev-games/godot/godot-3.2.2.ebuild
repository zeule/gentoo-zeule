...
src_compile() {
	if use mono; then
		einfo "Building first without mono-glue..."
		escons "${myesconsargs[@]}" mono_glue=no
		einfo "Generating mono-glue..."
		bin/godot.x11.opt.tools.64.mono --generate-mono-glue modules/mono/glue
		einfo "Building with mono-glue..."
		escons "${myesconsargs[@]}" mono_glue=yes
	else
		escons "${myesconsargs[@]}"
	fi
}
